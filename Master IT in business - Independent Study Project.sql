...
SELECT distinct(T1.Maintenance_ID)
      ...
      ,T1.LicensePlate
      ,T2.เลขไมล์ปัจจุบัน
	  ,LAG(T2.เลขไมล์ปัจจุบัน, 1) OVER(PARTITION BY T1.Service_ID,T1.Customer_ID,T1.LicensePlate
...
SELECT	Maintenance_ID
		...
		,เลขไมล์ปัจจุบัน
		,Prev_mile
		,(เลขไมล์ปัจจุบัน - Prev_mile) as diff_distance
FROM temp_1
WHERE Prev_mile is not null 
and เลขไมล์ปัจจุบัน <> Prev_mile
and เลขไมล์ปัจจุบัน > Prev_mile
and (เลขไมล์ปัจจุบัน - Prev_mile) < 100000
   --ORDER BY 8